...
class Cabbages():
    ...
    def generateCabbage(self, n, herd):
        ...
        while count < n:
            ...
            value = random.randint(5, herd.size * 2)  # задаем размер капусты от 5 до двойного размера стада, чтобы капуста не была слишком огромной
...
def findNearestCabbage(herd, cabbages): # определение ближайшей капусты, которую стадо должно съесть
    ...
    for cabbage in cabbages.cabbages: # перебираем каждый кочан капусты
        distance = ((herd.x - cabbage.x) ** 2 + (herd.y - cabbage.y) ** 2) ** 0.5 # вычисляем расстояние от капусты до стада по Пифагору
...
def update(canvas, herd, cabbages):
    ...
    if herd.eatingRightNow: # при поедании капусты стадом рисуем полукруг стада - синий
        canvas.create_arc(herd.x - herd.size, herd.y - herd.size,
                           herd.x + herd.size, herd.y + herd.size, start=90, extent=180, fill='blue')
    else:
        canvas.create_oval(herd.x - herd.size, herd.y - herd.size,
                       herd.x + herd.size, herd.y + herd.size, fill='blue') # если же не происходит процесса поедания, то стадо заполняется полностью синим цветом
...
cabbages.generateCabbage(17, herd)
...
while True: # создаем бесконечный цикл, в котором стадо ищет ближайщую капусту и направляется к ней, чтобы съесть
    ...
    herd.move(nearest_Cabbage.x, nearest_Cabbage.y)
    herd.updatePosition()

    if herd.x == nearest_Cabbage.x and herd.y == nearest_Cabbage.y: # если стадо достигает капусты, то начинается процесс поедания
        herd.eat(cabbages, nearest_Cabbage)

    update(canvas, herd, cabbages) # обновление холста
