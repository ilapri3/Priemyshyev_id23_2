...
class Cabbage:
    def __init__(self, x, y, val):
        ...
        self.x = x
        self.y = y
        self.val = val #задаем объем капусты
...
class Cabbages:
    ...
    def check_overlap(self, x, y, val): #делаем проверку на то, чтобы наши капусты не наростали друг на друга
        for cabbage in self.cabbages: # проходимся по уже созданным капустам
            ...
            if distance < (val + cabbage.val): # сравниваем расстояние между центрами
    ...
    def generateCabbage(self, n, goat):
        ...
        while count < n:
            ...
            val = random.randint(5, goat.size * 2)  # задаем размер капусты от 5 до двойного размера стада, чтобы капуста не была слишком огромной
            if not self.check_overlap(x, y, val): # проверка на то, чтобы капусты не пересекались между собой
    ...
    def appendCabbage(self, goat):
        while True: # цикл до тех пор, пока на огороде не появится новая капуста
            ...
            if not self.check_overlap(x, y, val): 
                break
        self.cabbages.append(Cabbage(x, y, val)) # если все удовлетворяет, то создаем объект и доваляем в массив, в котором есть все капусты
...
class Goat: # описываем поведение стада
    ...
    def eat(self, cabbages, cabbage): # процесс поедания капусты стадом
        if cabbage.val > 0: # проврерка, съедена капуста или нет
            cabbage.val -= self.eating # уменьшение размера капусты, в процессе поедания со скоростью поедания стада
            if cabbage.val <= 0: # условие, если капуста была съедена
            ...
            self.size += cabbage.val * self.fertility #увеличение размера стада после поедания капусты, в зависимости от количества съеденного и плодовитости
...
def findNearestCabbage(goat, cabbages): # определение ближайшей капусты, которую стадо должно съесть
    ...
    for cabbage in cabbages.cabbages: # перебираем каждый кочан капусты
        ...
        if distance < min and cabbage.val > 0: # если расстояние до куста меньше, чем минимальное расстояние и если капуста существует
            min = distance
...
def update(canvas, goat, cabbages):
    ...
    if goat.eatingRightNow: # при поедании капусты стадом рисуем два полукруга, один из которых серый, а другой - зеленый.
        ...
        canvas.create_oval(goat.x - goat.size, goat.y - goat.size,
                       goat.x + goat.size, goat.y + goat.size, fill='gray') # если же не происходит процесса поедания, то стадо заполняется полностью серым цветом
...
goat = Goat(speed=5, endurance=10, eating=2, fertility=0.06) # задание основных параметров для стада 
